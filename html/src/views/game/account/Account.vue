      ...
      <div class="q-ma-sm row">
        <q-card class="bg-secondary full-width row">
          <q-card-section class="col-12">
            <div class="text-h6">
              <span class="text-primary">{{ accountName }}</span>
            </div>
          </q-card-section>
        </q-card>
        <q-card class="bg-secondary full-width row q-mt-md">
          <div class="col-12">
            <q-table
              ref="tableRef"
              color="primary"
...
export default {
  ...
  methods: {
    getRowsNumberCount(filter) {
      return this.$axios
        .post('/api/accounts/account-player-count', {
    ...
    fetchFromServer(startRow, count, filter, sortBy, descending) {
      return this.$axios
        .post(`/api/accounts/${this.$route.params.license}`, {
          startRow: startRow,
          count: count,
          filter: filter,
          sortBy: sortBy,
    ...
    async onRequest(props) {
      ...
      if (!this.loaded) {
        this.accountName = returnedData[0].name
        this.loaded = true
      }
