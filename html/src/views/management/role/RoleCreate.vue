      ...
      <div class="q-ma-sm row">
        <q-card class="bg-secondary full-width row">
          <q-card-section class="col-12">
            <div class="text-h6">{{ $t(`components.roleCreate.new`) }}:</div>
          </q-card-section>
      ...
      this.$refs.roleForm.validate().then(success => {
        if (!success) {
          this.$q.notify({
            ...
            caption: this.$i18n.t(`components.role.msg.error.missingFields`, {
              name: this.user.name,
            }),
          })
        ...
        this.$axios
          .post(`/api/roles/create`, this.role)
          .then(r => {
